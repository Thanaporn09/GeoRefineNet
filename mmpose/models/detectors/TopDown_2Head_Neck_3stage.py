...
class TopDown_2Head_Neck_3stage(BasePose):
    ...
    @auto_fp16(apply_to=('img', 'attention_heatmap' ))
    def forward(self,
                img,
                target=None,
                target_weight=None,
        ...
        """
        if return_loss:
            return self.forward_train(img, target, target_weight, target_reg, target_weight_reg, attention_heatmap, img_metas,
                                      **kwargs)
        return self.forward_test(
            img, img_metas, attention_heatmap, return_heatmap=return_heatmap, **kwargs)

    def forward_train(self, img, target, target_weight,target_reg, target_weight_reg, attention_heatmap, img_metas, **kwargs):
        """Defines the computation performed at every call when training."""
        features = self.backbone(img, attention_heatmap)
    ...
    def forward_test(self, img, img_metas, attention_heatmap, return_heatmap=True, **kwargs):
        ...
        if self.test_cfg.get('flip_test', True):
            heatmap_flipped = attention_heatmap.flip(3)
            img_flipped = img.flip(3)
            features_flipped = self.backbone(img_flipped, heatmap_flipped)
            if self.with_neck:
...
#               print('(x,y):',x,' , ',y) 
               img = cv2.drawMarker(img, (int(float(x)),int(float(y))), color=(0, 205, 0), 
                                        markerType=cv2.MARKER_TILTED_CROSS, thickness=2, markerSize=20)
